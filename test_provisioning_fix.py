...
if response.status_code == 200:
    ...
    if 'complete_schema' in result:
        print(f"📊 Complete schema keys: {list(result['complete_schema'].keys())}")
        ...
        if complete_sql_parts:
            ...
            with open('data/test_generated_dw_fixed.sql', 'w') as f:
                f.write(complete_sql)
            print(f"💾 Complete SQL saved to data/test_generated_dw_fixed.sql ({len(complete_sql)} characters)")

            # Test provisioning
            print("\n🔄 Step 3: Testing provisioning with generated SQL...")
            session_response = requests.post('http://localhost:5001/api/nlq/session/create',
                                           json={"metadata": {"test": "csv-data-extraction-fixed"}})
            if session_response.status_code == 200:
                ...
                if provision_response.status_code == 200:
                    print("🎉 Provisioning successful! The complete workflow is working!")
                    print("✅ CSV upload → SQL generation → Star Schema generation → Provisioning: ALL WORKING!")
                else:
        ...
        print("❌ No complete_schema found in response")
